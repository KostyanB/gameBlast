...
class DrawGameField {
    ...
    //
    createField() {
        for(let line of this.fieldMap.keys()) this.addLine(line);
        for(let col of this.fieldMap.values()) col.forEach(item => this.addCell(item));
    }
	...
	addCell(args) {
		...
		const currentLine = document.querySelector(`.line-${posX}`);
